...
4.  **Phase 1: Generate Parent Tasks:** Based on the PRD analysis and current state assessment, create the file and generate the main, high-level tasks required to implement the feature. Use your judgement on how many high-level tasks to use. It's likely to be about five tasks. **IMPORTANT:** If the PRD includes core type definitions, include Task 0.0 as the first task for reviewing/finalizing them. If the PRD notes types were omitted (no storage requirements), skip Task 0.0 and note why in the task list. Present these tasks to the user in the specified format (without sub-tasks yet). Inform the user: "I have generated the high-level tasks based on the PRD. Ready to generate the sub-tasks? Respond with 'Go' to proceed."
...
<!-- Include Task 0.0 only if the PRD includes type definitions. Omit if PRD notes "Types omitted - no storage requirements" -->
- [ ] 0.0 Review and finalize core type definitions
  - [ ] 0.1 Review proposed core type definitions from PRD (types should use strong references, not IDs)
  - [ ] 0.2 Approve or request modifications to type structure
  - [ ] 0.3 Note: Serialization DTOs will be created later and are not part of this task

<!-- If Task 0.0 is omitted, add a note like: "Task 0.0 omitted - no storage requirements for this feature" -->
